...
### Добавлено
- **Архитектура All-In-One GPU**: Кардинальное изменение стратегии управления памятью. Теперь все ключевые модели (Whisper, Diarization, LLM) загружаются в VRAM один раз и остаются там на протяжении всей сессии.
- **Diarization 3.1 (CUDA)**: Полная поддержка GPU-акселерации для этапа разделения спикеров. Время анализа 10-минутной записи сокращено с 7 минут до ~45 секунд.
- **Ультра-стабильность на Windows**: Полностью устранены критические ошибки `libtorchcodec_coreX.dll` и `FileNotFoundError` при выгрузке моделей, которые ранее приводили к падению бэкенда.
- **Умные повторы (Robust Ollama)**: Реализована логика автоматического восстановления при пустых или циклических ответах локальных LLM без перезапуска всего пайплайна.

### Улучшено
- **Производительность**: Общее время обработки (End-to-End) сокращено в **2.3 раза** (с 13.3 до 5.7 минут для 10-минутных аудио на RTX 3060).
- **Точность транскрибации**: Стандартная конфигурация теперь ориентирована на Whisper `medium` (int8_float16) для обеспечения высокого качества распознавания при сохранении лимита в 12GB VRAM.

### Исправлено
- **Критический баг с импортом**: Исправлена ошибка `name 'gc' is not defined` в основном конвейере событий.
- **Конфликты VRAM**: Решена проблема фрагментации памяти при интенсивной пересменке задач STT -> Diarization.
