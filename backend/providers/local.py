...
class LocalProvider(BaseAIProvider):
    ...
    async def transcribe_audio(
        ...
        if self.device == "cuda":
            ...
            await asyncio.sleep(1)  # Даем системе "выдохнуть"
    ...
    async def _summarize_chunk(self, chunk: str, index: int, total: int, trace: Any = None, context: Optional[str] = None) -> str:
        fallback_system = f"Ты — профессиональный секретарь. Выдели главное из части {index} (из {total}). КАТЕГОРИЧЕСКИ ЗАПРЕЩЕНО писать проценты уверенности. Если Conf < 65%, просто добавь в конце '(требуется уточнение)'."
    ...
    async def _create_protocol_single(self, text: str, is_consolidated: bool = False, trace: Any = None, context: Optional[str] = None) -> Dict[str, Any]:
        ...
        fallback_system = "Ты — профессиональный секретарь. Составь протокол на русском языке. ЗАПРЕЩЕНО выдумывать сроки. КАТЕГОРИЧЕСКИ ЗАПРЕЩЕНО писать проценты в итоговом тексте. Если Conf < 65%, добавь в конце пункта '(требуется уточнение)'."
    ...
    async def _summarize_for_audit(self, transcription: str, context: Optional[str] = None, trace: Any = None) -> str:
        logger.info(f"Summarizing long transcript for audit (local, length: {len(transcription)})")
